                ...
                for (const char* p = (const char*)buf; *p; p += std::strlen(p) + 1) {
                    ...
                    if (vp && pp) {
                        ...
                        if (sscanf(vp + 4, "%4x", &vid) == 1 && sscanf(pp + 4, "%4x", &pid) == 1 && vid == 0x35CA) {
                            if (xr_device_provider_is_product_id_valid((int)pid)) { // NEW VALIDATION
    ...
    static bool autodetect_viture_usb(std::vector<int>& out_pids) {
        ...
        for (hid_device_info* c = devs; c; c = c->next)
            if ((int)c->vendor_id == 0x35CA) {
                ...
                if (xr_device_provider_is_product_id_valid(pid)) {
                    if (std::find(out_pids.begin(), out_pids.end(), pid) == out_pids.end())
                        out_pids.push_back(pid);
                }
            }
...
bool Viture::is_carina() const {
    return m_connected && m_device_type == XR_DEVICE_TYPE_VITURE_CARINA;
}
...
void Viture::get_imu_data(float euler_out[3], float quat_out[4], float gyro_out[3]) {
    ...
    const float* r = m_3dof_ref_inv_quat;  // [w, 0, 0, z]
    const float* q = m_fused_quat;          // [w, x, y, z]

    float rw = r[0], rz = r[3];
    quat_out[0] = rw * q[0] - rz * q[3];  // w
    quat_out[1] = rw * q[1] - rz * q[2];  // x
    quat_out[2] = rw * q[2] + rz * q[1];  // y
    quat_out[3] = rw * q[3] + rz * q[0];  // z
}
...
bool Viture::get_6dof_pose(float quat_out[4], float pos_out[3], bool* stable_out) {
    std::lock_guard<std::mutex> lk(m_6dof_mutex);
    if (!m_carina_fresh) return false;

    if (!m_6dof_recentered) {
        recenter_6dof();
    }

    float rq[4] = { m_carina_quat[0], m_carina_quat[1], m_carina_quat[2], m_carina_quat[3] };
    ...
    float rel_quat[4];
    qmul(m_ref_inv_quat, rq, rel_quat);

    quat_out[0] = rel_quat[0];
    quat_out[1] = rel_quat[1];
    quat_out[2] = rel_quat[2];
    quat_out[3] = rel_quat[3];

    float dx = m_carina_pos[0] - m_ref_pos[0];
    float dy = m_carina_pos[1] - m_ref_pos[1];
    float dz = m_carina_pos[2] - m_ref_pos[2];

    float w = m_ref_inv_quat[0];
    float qx = m_ref_inv_quat[1], qy = m_ref_inv_quat[2], qz = m_ref_inv_quat[3];
    float tx = 2.f * (qy * dz - qz * dy);
    float ty = 2.f * (qz * dx - qx * dz);
    float tz = 2.f * (qx * dy - qy * dx);
    pos_out[0] = dx + w * tx + (qy * tz - qz * ty);
    pos_out[1] = dy + w * ty + (qz * tx - qx * tz);
    pos_out[2] = dz + w * tz + (qx * ty - qy * tx);

    if (stable_out) *stable_out = (m_carina_status == 0);
    return true;
}

void Viture::recenter_6dof() {
    m_ref_pos[0] = m_carina_pos[0];
    m_ref_pos[1] = m_carina_pos[1];
    m_ref_pos[2] = m_carina_pos[2];

    float w = m_carina_quat[0];
    float y = m_carina_quat[2];
    float yaw_len = std::sqrt(w * w + y * y);
    if (yaw_len < 1e-6f) { yaw_len = 1.f; }
    float yaw_w = w / yaw_len;
    float yaw_y = y / yaw_len;
    
    m_ref_inv_quat[0] = yaw_w;
    m_ref_inv_quat[1] = 0.f;
    m_ref_inv_quat[2] = -yaw_y;
    m_ref_inv_quat[3] = 0.f;

    m_6dof_recentered = true;
}
...
void Viture::monitor_thread_func() {
    while (m_running) {
        if (!m_connected) {
            ...
            autodetect_viture_usb_fallback(pids);

            if (!pids.empty()) {
                int best_pid = pids[0];
                int best_type = 0;
                for (int p : pids) {
                    void* h = xr_device_provider_create(p);
                    if (!h) continue;
                    int t = xr_device_provider_get_device_type(h);
                    xr_device_provider_destroy(h);

                    if (t > best_type) { best_type = t; best_pid = p; }
                }
                ...
                if (!m_device_handle) {
                    ...
                    m_device_type = xr_device_provider_get_device_type(m_device_handle);                    if (m_device_type == XR_DEVICE_TYPE_VITURE_GEN1 ||
                    ...
                    if (m_device_type == XR_DEVICE_TYPE_VITURE_CARINA) {
                        xr_device_provider_set_dof_type_carina(m_device_handle, 1);
                    }

                    int init_ret = xr_device_provider_initialize(m_device_handle, nullptr, nullptr);
                    int start_ret = (init_ret == 0) ? xr_device_provider_start(m_device_handle) : -1;
            ...
            if (m_device_type == XR_DEVICE_TYPE_VITURE_CARINA) {
                ...
                int pose_ret = xr_device_provider_get_gl_pose_carina(m_device_handle, pose, 0.0, &pose_status);

                if (pose_ret == 0) {
                    m_quat_atomic[0] = pose[3]; m_quat_atomic[1] = pose[4];
                    m_quat_atomic[2] = pose[5]; m_quat_atomic[3] = pose[6];
                    m_raw_data_fresh.store(true, std::memory_order_release);
                    m_last_imu_time = std::chrono::steady_clock::now();

                    {
                        std::lock_guard<std::mutex> lk(m_6dof_mutex);
                        m_carina_pos[0] = pose[0];
                        m_carina_pos[1] = pose[1];
                        m_carina_pos[2] = pose[2];
                        m_carina_quat[0] = pose[3];
                        m_carina_quat[1] = pose[4];
                        m_carina_quat[2] = pose[5];
                        m_carina_quat[3] = pose[6];
                        m_carina_status = pose_status;
                        m_carina_fresh = true;
                    }
                }
...
void Viture::reset_imu_data() {
    ...
    std::lock_guard<std::mutex> lk6(m_6dof_mutex);
    m_carina_quat[0] = 0; m_carina_quat[1] = 0; m_carina_quat[2] = 0; m_carina_quat[3] = 1;
    m_carina_pos[0] = 0; m_carina_pos[1] = 0; m_carina_pos[2] = 0;
    m_carina_status = 1;
    m_carina_fresh = false;
    m_6dof_recentered = false;
    m_ref_inv_quat[0] = 1; m_ref_inv_quat[1] = 0; m_ref_inv_quat[2] = 0; m_ref_inv_quat[3] = 0;
    m_ref_pos[0] = 0; m_ref_pos[1] = 0; m_ref_pos[2] = 0;
}

void Viture::reset_origin_carina() {
    std::lock_guard<std::mutex> lk(m_6dof_mutex);

    if (m_device_type != XR_DEVICE_TYPE_VITURE_CARINA || !m_device_handle || !m_carina_fresh) {
        return;
    }

    float origin[7] = {
        m_carina_pos[0], m_carina_pos[1], m_carina_pos[2],
        m_carina_quat[0], m_carina_quat[1], m_carina_quat[2], m_carina_quat[3]
    };

    xr_device_provider_reset_origin_carina(m_device_handle, origin);

    m_ref_pos[0] = 0.f;
    m_ref_pos[1] = 0.f;
    m_ref_pos[2] = 0.f;

    m_ref_inv_quat[0] = 1.f;
    m_ref_inv_quat[1] = 0.f;
    m_ref_inv_quat[2] = 0.f;
    m_ref_inv_quat[3] = 0.f;

    m_6dof_recentered = true;
}

void Viture::recenter_3dof() {
    std::lock_guard<std::mutex> lk(m_imu_mutex);

    float w = m_fused_quat[0];
    float z = m_fused_quat[3];

    float yaw_len = std::sqrt(w * w + z * z);
    if (yaw_len < 1e-6f) return;

    float yaw_w = w / yaw_len;
    float yaw_z = z / yaw_len;

    m_3dof_ref_inv_quat[0] = yaw_w;
    m_3dof_ref_inv_quat[1] = 0.f;
    m_3dof_ref_inv_quat[2] = 0.f;
    m_3dof_ref_inv_quat[3] = -yaw_z;

    m_3dof_recentered = true;
}