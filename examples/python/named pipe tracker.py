# named pipe example script for trackers:
...
#each tracker uses its own pipe, change the index to the tracker you want to send data to, 0...63 (steamvr limit)
tracker_index = "0"
...
while True:
    h_pos = create_pipe(f"\\\\.\\pipe\\GlassVR_TRACKER_{tracker_index}_Pos")
    h_rot = create_pipe(f"\\\\.\\pipe\\GlassVR_TRACKER_{tracker_index}_Rot")
